...
**Tagline + "?" button** (reversal-prone): `fitTagline()` shrinks the hero tagline to **exactly 2 lines on mobile** in every language (runs on render, resize, language switch); it's plain `.tagline` text — dark in light mode, light with a soft shadow at night. (An earlier white cloud *behind* the tagline — `tagCloud()`/`.tag-in` — was tried and then removed; don't reintroduce it. The wordmark clouds `wordCloud()` are separate and stay.) The Chapters info button is a **`?`** (was `i`, which mistranslated to "me"); the symbol is `data-notranslate` so it stays `?`, but the tooltip still translates.

**Analytics / roles** (reversal-prone): new **`views_tracked`** cap (in `CAPS`, defaults ON via an `allCaps` special-case; loaded roles are backfilled). `viewerTracked()` gates `logPageView` + `logLangView` off the *real* account role, so a role with it off (e.g. a beta-tester role) isn't counted. The RTL modal close button floats left (`[dir="rtl"] .modal-card .x`).

**Language views — all-time + weekly** (reversal-prone): `ese_lang_stats()` returns `{all_time, week}` per language — `all_time` = count of every view (durable), `week` = views since `date_trunc('week', now())` (Monday). The weekly figure **auto-resets each week purely via the date window** — no rows are deleted, so the weekly reset can never touch all-time. Only a **manual clear** removes views: `ese_clear_lang_views()` wipes all (uses `delete … where true` to dodge safe-delete guards), `ese_clear_lang_view(p_lang)` wipes one language — both owner-gated, both in `supabase-setup.sql`, powering the "Clear all language views" button + the per-row **×** (`clearOneLangView`). (If the Supabase *dashboard* "delete all rows" is used instead, PostgREST demands a WHERE clause — that's the dashboard, not the app; use the in-app button.) These functions get dropped when `supabase-setup.sql` is regenerated — re-apply them live if the button/stats break.

**Applications — view ≠ delete** (reversal-prone): in Owner Analytics, clicking an application **only views it** (`viewApplication` no longer calls `markReviewed`). Reviewed ones move to a collapsible **"Reviewed applications"** section and stay **revisitable**; the view modal has Mark reviewed / Move back + **Delete**. Deletion is explicit (`deleteApplication`→confirm→`doDeleteApplication`) and needs the admin **delete** RLS policies on `chapter_applications` + `youth_applications` (in `supabase-setup.sql`).
...
**Logo** (updated 2026-07-22): the brand is the **green + blue "ESE" butterfly** (green wings forming E's, a blue S). The icon PNGs + `favicon.ico` + `site.webmanifest` are the new logo; the nav's **embedded base64 `.brand-logo`** (in `index.html`) and the `ESE Logo.png` 600×600 source were updated to match (they're separate copies — when the logo changes, update the embedded one too, it won't follow the PNG files). The new logo has a **transparent** background (sits on the yellow/brown topbar cleanly). Manifest `theme_color`=`#F4D462`, `background_color`=`#D5E3DA`. Note: `og:image` points to `https://earthsweetearth.org/social.jpg`, which **is not in the repo** (link-preview image 404s) — pre-existing, needs a real social card.

**Always verify before commit:** render with **0 JS exceptions**, and confirm the anon key + Supabase base URL are unchanged (the embedded logo is now the new green/blue butterfly — see the Logo note).
