...
**Tagline + "?" button** (reversal-prone): the hero tagline sits on a **static white cloud** (`.tag-in` wraps the text, `tagCloud()` draws it, dark `#20302a` text in light mode; **at night the cloud is hidden and the text goes light** over the night sky) and `fitTagline()` shrinks it to **exactly 2 lines on mobile** in every language (runs on render, resize, language switch). The Chapters info button is a **`?`** (was `i`, which mistranslated to "me"); the symbol is `data-notranslate` so it stays `?`, but the tooltip still translates.
